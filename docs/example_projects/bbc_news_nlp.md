...
* `Aggregating texts`: aggregates article summaries and groups them by source (summaries are used instead of titles since they tend to be longer)
...
Finally, we can add the `dim_bbc_feeds_nlp_stats` task to the the `modelling` group. Like the previous modelling task, we will create this task using the modelling preset in `project.yaml`; setting the parents parameter to `nlp`. We will also change the materialisation to a `view`, since we only need to view the statistics.
