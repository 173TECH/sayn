...
!!! tip
    ...
    We all love `pandas`! If you want to load a pandas dataframe with the `self.default_db.load_data` method, you can prepare the data as such: `data = [v for _, v in df.to_dict('index').items()]`.
