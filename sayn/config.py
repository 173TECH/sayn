...
class Config:
    ...
    def _read_project(self):
        ...
        try:
            ...
            UI()._error(f"Error reading {path.name}")
            UI()._error(f"{e}")
            return
        ...
        if default_db is None and len(parsed["required_credentials"]) == 1:
            ...
            UI()._error(f'Default database "{default_db}" not defined')
            return
        # else:
    ...
    def _read_dag(self, dag_name):
        ...
        try:
            ...
            UI()._error(f'Error reading dag "{path.name}"')
            UI()._error(f"{e}")
            return
    ...
    def _read_settings(self, profile, required_credentials, allowed_parameters):
        ...
        if len(env_vars) > 0:
            ...
            UI()._info("Reading settings from environment variables")
            return self._read_settings_environment(
                env_vars, required_credentials, allowed_parameters
            )
        else:
            UI()._info('Reading settings from "settings.yaml"')
    ...
    def _read_settings_file(self, profile, required_credentials, allowed_parameters):
        ...
        try:
            ...
            UI()._error(f'Error reading settings "{path.name}"')
            UI()._error(f"{e}")
            return
        ...
        if selected_profile is None and len(parsed["profiles"]) > 1:
            UI()._error("Default profile is required when multiple profiles are set")
            return
            ...
            UI()._error(f'Default profile "{selected_profile}" not defined')
            return
        ...
        if len(no_type) > 0:
            UI()._error(f"Some credentials have no type: {', '.join(no_type)}")
            return
        credentials = {
    ...
    def _setup_python_tasks_module(self):
        ...
        try:
            ...
            UI()._error(f"Error importing python module")
            UI()._error(f"{e}")
        ...
        for name, task in tasks.items():
            ...
            if "type" not in task:
                if "type" in task.get("preset", dict()):
                    ...
                    UI()._error(f"Missing type in task {name} in dag {dag_name}")
                    return
